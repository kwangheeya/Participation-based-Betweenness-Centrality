...
def sBBC_compute(n, e, possible_nodes=[], eps=0.01, delta=0.1, op='v2', lam=0.5, p_op=False, printFrequency=1000):
    ...
    print('\t Initial sample ', next_num_samples, ' / total ', total_num_samples)
    if next_num_samples >= total_num_samples * 0.5:
    ...
    elif len(possible_nodes) >= len(e):
        print('Too small hyperedges.',len(possible_nodes),len(e),' Turn to aBBC')
        bc = bc_hyper_single(n, e, p_op=p_op, lam=lam, op=op)
    else:
        ...
        while(True):
            ...
            debug_print('\t# of samples', next_num_samples, print_op=p_op)
            if delta_s < eps:
                break
...
def sBBC_main(dataFile='../data/test/test.txt', eps=[0.01], delta=0.1, p_op=True, mode='a'):
    ...
    for epsilon in eps:
        bc = sBBC_compute(n, e, eps=epsilon, delta=delta, p_op=p_op)
